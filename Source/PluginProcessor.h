...
// Premier ordre all-pass pour Uni-Vibe (4 étages)
struct VibeStage {
...
public:
    ...
    static constexpr int FREEZE_BUF  = 176400;  // ~4s @ 44100
    ...
    // Freeze — appelé depuis le thread UI
    void setFreezeActive(bool on);
    bool isFreezing() const { return freezeActive.load(); }

    // Specter filter — mis à jour par la souris dans SpecterPad
...
private:
    ...
    // Uni-Vibe (4 étages allpass + LFO)
    VibeStage vibeL[4], vibeR[4];
    float vibeLfoPhase=0.f;

    // Spectral Freeze (loop buffer + shimmer)
    ...
    // Shimmer : deuxième tête de lecture à vitesse ×2
    float shimReadL=0.f, shimReadR=0.f;
    float shimXL=0.f, shimXR=0.f;   // crossfade phase [0-1]
    bool  prevFreezeState=false;
    std::atomic<bool> freezeActive{false};

    // Specter Filter (biquad 2nd order)
    ...
    float chaosOffset=0.f; // LFO aléatoire pour shape CHAOS
