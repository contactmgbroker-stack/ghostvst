...
namespace C {
    ...
    const Colour violet{ 0xFFAA77FF };
    const Colour freeze{ 0xFF88DDFF };
                                         ...
                                         float pos,float start,float end,Slider& sl)
{
    ...
    // ── Live highlight ring (random colour stored per slider) ───────────────
    int rank=(int)sl.getProperties()["live"];
    if(rank>0){
        // Read the colour stored by updateLiveHighlights
...
void WaveformDisplay::paint(Graphics& g)
{
    ...
    // Oscilloscope clippé dans la vague
...
void SpecterPad::timerCallback()
{
    ...
    else repaint(); // hover cursor always fresh
}

// drawFilterCurve uses filterX/filterY (committed position, not hover)
void SpecterPad::drawFilterCurve(Graphics& g,Rectangle<float> area,int shape)
{
    ...
    float fc=filterX;   // committed cutoff (0-1)
    float q =1.f-filterY; // committed resonance (0-1)

    Path curve; bool started=false;
    const int NPTS=200;
    for(int i=0;i<NPTS;++i){
        float t=(float)i/NPTS;
        float resp=0.f;
        switch(shape){
        case 0: { // SINE - lowpass
            float dist=t-fc;
            float peak=q*0.8f*std::exp(-dist*dist*8.f);
            resp=(t<fc?1.f:std::exp(-(t-fc)*8.f))+peak;
            break; }
        case 1: { // SQUARE - bandpass
            float bw=0.05f+q*0.12f;
            float dist=std::abs(t-fc);
        ...
        case 3: { // CHAOS - lowpass + random notches
            resp=(t<fc?1.f:std::exp(-(t-fc)*8.f));
            float notch=0.4f*std::sin(t*42.f+fc*10.f)*std::sin(t*17.f);
...
void SpecterPad::paint(Graphics& g)
{
    ...
    // Courbe filtre (suit filterX/filterY — position commitée uniquement)
    ...
    // Curseur hover (suit la souris, croix + halo)
    ...
    // Point committé (filterX/Y) — différent du curseur hover
    float fx=b.getX()+filterX*W, fy=b.getY()+filterY*H;
    ColourGradient glow(C::aqua.withAlpha(0.50f),fx,fy,C::aqua.withAlpha(0.f),fx+18,fy,true);
    g.setGradientFill(glow); g.fillEllipse(fx-18,fy-18,36,36);
    g.setColour(C::ice); g.fillEllipse(fx-4,fy-4,8,8);
    g.setColour(C::aqua.withAlpha(0.85f)); g.drawEllipse(fx-5,fy-5,10,10,1.5f);
    // Petit dot au curseur hover si différent du committé
    if(std::abs(curX-filterX)>0.02f || std::abs(curY-filterY)>0.02f){
        g.setColour(C::sky.withAlpha(0.40f));
        g.fillEllipse(cx-3,cy-3,6,6);
    }

    // Boutons forme (SINE / SQ / SAW / CHAOS)
    ...
    // Cutoff + résonance label
    g.setColour(C::dim); g.setFont(Font("Arial",8.f,Font::plain));
    float fc2=proc.getSpecterCutoff();
    String fcStr=fc2<1000.f?String((int)fc2)+"Hz":String(fc2/1000.f,1)+"kHz";
...
void SpecterPad::mouseDown(const MouseEvent& e)
{
    ...
    dragging=true;
    // Mettre à jour curseur hover ET position committée
    curX=jlimit(0.f,1.f,(e.x-b.getX())/b.getWidth());
    curY=jlimit(0.f,1.f,(e.y-b.getY())/b.getHeight());
    filterX=curX; filterY=curY;
    float fc=20.f*std::pow(1000.f,filterX);
...
{
    // Mise à jour hover uniquement — ne touche pas filterX/Y ni le processeur
    auto b=getLocalBounds().toFloat();
    curX=jlimit(0.f,1.f,(e.x-b.getX())/b.getWidth());
    curY=jlimit(0.f,1.f,(e.y-b.getY())/b.getHeight());
}
...
void VUMeter::timerCallback()
{
    ...
    // Attaque rapide / relâche lente — évite que le mètre monte trop vite
    float coeff = (target > displayLevel) ? 0.4f : 0.06f;
...
void VUMeter::paint(Graphics& g)
{
    ...
    // Plage -36 à 0 dBFS — moins sensible pour les signaux moyens
...
void GhostSurfEditor::buildKnob(KnobWidget& kw,const char* id,const char* lbl,Colour ac)
{
    ...
    // Mode direct : réponse 1:1 au drag — pas de vélocité
    ...
    : AudioProcessorEditor(&p),proc(p),vuMeter(p),waveDisplay(p),specterPad(p),freezePanel(p)
{
    setSize(760,650);
    setLookAndFeel(&lf);
    setWantsKeyboardFocus(true); // navigation clavier

    // Couleurs live initiales
    liveColours[0]=C::sky; liveColours[1]=C::aqua; liveColours[2]=C::mint;
    ...
    // ── Flanger knobs (section basse du panneau TREMOLO)
    ...
    // ── Wah knobs (section basse du panneau EFFETS)
...
// ── Keyboard navigation ───────────────────────────────────────────────────────
bool GhostSurfEditor::keyPressed(const juce::KeyPress& k)
{
    // ── UP / DOWN : changer de preset ─────────────────────────────────────────
    if(k == KeyPress::upKey){
        int p=(proc.getCurrentProgram()-1+GhostSurfProcessor::NUM_PRESETS)
               % GhostSurfProcessor::NUM_PRESETS;
    ...
    // ── LEFT / RIGHT : ajuster le potard rank-1 de ±1% ───────────────────────
    if(k == KeyPress::leftKey || k == KeyPress::rightKey){
        float delta = (k == KeyPress::rightKey) ? 0.01f : -0.01f;

        // Trouver le slider avec live==1
        KnobWidget* knobs[]={
        ...
        for(auto* kw : knobs){
            if((int)kw->slider.getProperties()["live"] == 1){
                double cur = kw->slider.getValue();
                double mn  = kw->slider.getMinimum();
                double mx  = kw->slider.getMaximum();
                double step= (mx-mn)*delta;
                kw->slider.setValue(jlimit(mn,mx,cur+step), sendNotificationAsync);
                break;
            }
        }
        return true;
...
// ── updateLiveHighlights — couleurs aléatoires par preset ────────────────────
void GhostSurfEditor::updateLiveHighlights(int idx)
{
    ...
    static const Def L[12]={
        ...
        {"drive",        "vibeDepth",      "wahDepth"},     // Jimi Hendrix
        {"treble",       "bass",           "lofi"},         // Nile Rodgers
    };

    // Générer 3 couleurs aléatoires saturées (teintes distinctes)
    auto& rnd=juce::Random::getSystemRandom();
    float h0=rnd.nextFloat();
    liveColours[0]=Colour::fromHSV(h0,                      0.85f,1.00f,1.f);
    liveColours[1]=Colour::fromHSV(std::fmod(h0+0.33f,1.f), 0.80f,0.95f,1.f);
    liveColours[2]=Colour::fromHSV(std::fmod(h0+0.67f,1.f), 0.75f,0.90f,1.f);
    ...
    KM km[]={
        ...
        {"flangerRate",  &flangerRate.slider},
        {"flangerDepth", &flangerDepth.slider},
        {"flangerFeedback",&flangerFeedback.slider},
        {"drive",        &drive.slider},
        {"lofi",         &lofi.slider},
        {"bass",         &bass.slider},
        {"treble",       &treble.slider},
        {"wahDepth",     &wahDepth.slider},
        {"wahRate",      &wahRate.slider},
        {"slideAmount",  &slideAmount.slider},
        {"slideSpeed",   &slideSpeed.slider},
        {"vibeSpeed",    &vibeSpeed.slider},
        {"vibeDepth",    &vibeDepth.slider},
        {"freezeGrain",  &freezeGrain.slider},
        {"freezeShimmer",&freezeShimmer.slider},
        {"freezeDecay",  &freezeDecay.slider}
    };

    // Tout remettre à zéro
    for(auto& k:km){
        k.s->getProperties().set("live",0);
        k.s->getProperties().set("liveColor",(int64)0);
        k.s->repaint();
    }
    ...
    for(int r=0;r<3;++r)
        for(auto& k:km)
            if(std::strcmp(k.id,ranked[r])==0){
                k.s->getProperties().set("live",r+1);
                k.s->getProperties().set("liveColor",(int64)liveColours[r].getARGB());
                k.s->repaint();
            }
}
...
void GhostSurfEditor::paint(Graphics& g)
{
    ...
    drawScratchedTitle(g,"GHOST SURF",12.f,12.f);

    // ── SURF SCORE bar ──────────────────────────────────────────────────────
    {
        float barX=228.f,barY=20.f,barW=175.f,barH=11.f;
        ...
        g.setColour(C::dim); g.setFont(Font("Arial",8.f,Font::bold));
        g.drawText("SURF SCORE",(int)barX,(int)(barY+barH+2),(int)barW,10,Justification::centredLeft);
        if(comboFlash>1){
            float bx=barX+barW+6,by=barY-2;
            Colour cc=comboFlash>=6?Colour(0xFFFFDD00):comboFlash>=3?C::aqua:C::sky;
            g.setColour(cc.withAlpha(0.25f)); g.fillRoundedRectangle(bx,by,38,17,6.f);
            g.setColour(cc); g.drawRoundedRectangle(bx,by,38,17,6.f,1.2f);
            g.setFont(Font("Arial",8.f,Font::bold));
            g.drawText("x"+String(comboFlash),(int)bx,(int)by,38,17,Justification::centred);
        }
    }

    // ── Légende live (couleurs aléatoires actuelles) ────────────────────────
    for(int r=0;r<3;++r){
        g.setColour(liveColours[r].withAlpha(0.88f));
        g.fillEllipse((float)(W-148+(r)*40),22.f,8.f,8.f);
    }
    g.setColour(C::dim); g.setFont(Font("Arial",8.f,Font::plain));
    g.drawText("live",W-110,20,38,12,Justification::centredLeft);

    // ── Panneaux ────────────────────────────────────────────────────────────
    drawPanel(g,{8,  65,215,310},"SPRING REVERB", C::sky);
    drawPanel(g,{231,65,162,310},"TREMOLO + FLANGER", C::aqua);
    drawPanel(g,{401,65,234,310},"EFFETS + WAH",  C::cobalt);
    ...
    // Sous-séparateurs visuels dans TREMOLO
    g.setColour(C::aqua.withAlpha(0.14f));
    g.drawHorizontalLine(230,235.f,389.f);
    g.setColour(C::aqua.withAlpha(0.35f)); g.setFont(Font("Arial",8.f,Font::bold));
    g.drawText("— FLANGER —",231,222,162,12,Justification::centred);

    // Sous-séparateur dans EFFETS
    g.setColour(C::cobalt.withAlpha(0.14f));
    g.drawHorizontalLine(230,405.f,631.f);
    g.setColour(C::cobalt.withAlpha(0.35f)); g.setFont(Font("Arial",8.f,Font::bold));
    g.drawText("— WAH-WAH —",401,222,234,12,Justification::centred);
    ...
    // ── Hint clavier ────────────────────────────────────────────────────────
    g.setColour(C::dim.withAlpha(0.40f)); g.setFont(Font("Arial",7.f,Font::plain));
    g.drawText("↑↓ preset   ←→ knob",4,46,210,12,Justification::centredLeft);
}
...
void GhostSurfEditor::resized()
{
    ...
    tremSyncBtn.setBounds(240,197,62,22);
    tremDivBox .setBounds(307,197,80,22);
    vibeModeBtn.setBounds(649,344,106,22);

    const int KS=52, KY=125;

    // Spring Reverb (panneau gauche)
    ...
    // Tremolo (ligne haute du panneau TREMOLO)
    placeKnob(tremSpeed, 265, KY, KS);
    placeKnob(tremDepth, 351, KY, KS);

    // Flanger (ligne basse du panneau TREMOLO, sous le séparateur)
    placeKnob(flangerRate,     253, 265, 44);
    placeKnob(flangerDepth,    308, 265, 44);
    placeKnob(flangerFeedback, 363, 265, 44);

    // Effets (ligne haute du panneau EFFETS)
    placeKnob(drive,  449, KY, KS);
    placeKnob(lofi,   513, KY, KS);
    placeKnob(bass,   577, KY, KS);
    placeKnob(treble, 625, KY, KS);

    // Wah (ligne basse du panneau EFFETS, sous le séparateur)
    placeKnob(wahDepth, 449, 268, 44);
    placeKnob(wahRate,  509, 268, 44);
